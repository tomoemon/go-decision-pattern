...
- 構文を抜けた先へ複数の経路が合流する形は、条件が出せないことがある。経路は連言としてしか持てないため。条件が付かない遷移は `-shape=decision` ではひし形を通らない裸の線になり、`-shape=state` では「それ以外」というラベルになる
  - タグ付き `switch` を素通りする `case` は出せる。`case` は値で排他なので「脱出する `case` のどれにも当たらなかった」で言い切れる。ただし素通りが確定する本体に限る。本体が空、`case` の直下の `break`、`return` しない処理までで、`fallthrough`（次の `case` へ行く）、`panic`、`for`、`goto` があると確定しない
  - 全 `case` 式が定数のときに限る。定数でない式は実行時に重なりうるので上から順に評価され、タグなしと同じく順序を持つ
  - タグなし `switch` と `if` 連鎖は出せない。枝が順序を持ち、手前が素通りすると先の枝は条件が重なっていても評価されないので、「脱出する枝に当たらなかった」では言い表せない
  - `case` の中で一部だけ `return` する形も出せない。抜けた先に着くには `case` の中の分岐も要る
